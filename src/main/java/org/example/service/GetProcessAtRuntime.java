...
public class GetProcessAtRuntime {
    ...
    private static final String[] SEARCH_JAVAW_PROCESS_NAME_USING_WMIC = {"wmic", "Path," ,"win32_process", "Where", "\"CommandLine Like '%test.jar%'\""};
    private static final String[] SEARCH_JAVAW_PROCESS_NAME_USING_PS_ELF = { "/bin/sh", "-c", "ps -elf | grep -v grep | grep 'test.jar'" };

    public GetProcessAtRuntime() {
    ...
    public void execute() {
        ...
        if(osInfo == GetOperatingSystem.WINDOWS) {
            ...
            this.detectTestJarProcess((line) -> line.startsWith("javaw.exe") && line.contains(TEST_JAR), SEARCH_JAVAW_PROCESS_NAME_USING_WMIC);

        } else if(osInfo == GetOperatingSystem.LINUX) {

            this.detectTestJarProcess((line) ->  line.contains(TEST_JAR), SEARCH_JAVAW_PROCESS_NAME_USING_PS_ELF);
    ...
    private void detectTestJarProcess(Predicate<String> predicate, String ...command) {
